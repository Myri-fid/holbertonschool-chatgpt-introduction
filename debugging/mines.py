...
class Minesweeper:
    def __init__(self, width=10, height=10, mines=10):
        ...
        # Compteur des cellules non-mines à révéler
    ...
    def reveal(self, x, y):
        if (y * self.width + x) in self.mines:
            return False
        if not self.revealed[y][x]:  # Si la cellule n'a pas déjà été révélée
            self.revealed[y][x] = True
            self.non_mine_cells -= 1  # Réduire le compteur des cellules non-mines restantes
